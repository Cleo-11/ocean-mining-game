...
class MultiplayerService {
  ...
  connect(username: string, walletAddress: string) {
    if (!this.isClient) {
      console.warn("Multiplayer service can only run on client side")
      return
    }
  ...
  private generateMockPlayers() {
    ...
    setInterval(() => {
      if (this.offlineMode && this.isClient) {
  ...
  private generateMockResources() {
    ...
    setInterval(() => {
      if (this.offlineMode && this.resourceNodes.size < 30 && this.isClient) {
        const newNode = {
  ...
  getConnectionStatus() {
    if (!this.isClient) return "disconnected"
