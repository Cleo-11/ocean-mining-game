...
function FallbackGame({ walletConnected }: { walletConnected: boolean }) {
  return (
    <div className="flex h-full w-full items-center justify-center bg-gradient-to-b from-blue-900 to-blue-950">
      <div className="text-center">
        <div className="mb-4 text-4xl">🌊</div>
        <div className="mb-4 text-xl text-cyan-400">Ocean Mining Game</div>
        <div className="mb-6 text-slate-300">
          {walletConnected
            ? "3D environment loading... Please wait or refresh the page."
            : "Connect your wallet to start mining!"}
        </div>
        <button
          onClick={() => window.location.reload()}
          className="rounded-lg bg-gradient-to-r from-teal-500 to-cyan-600 px-4 py-2 font-medium text-white"
        >
          Reload Game
        </button>
      </div>
...
export function OceanMiningSimulation({
  ...
  }, [walletConnected, targetNode, setSidebarOpen, isClient])

  // Update player position based on movement keys
  useEffect(() => {
    if (!walletConnected || gameState === "mining" || !isClient) return
  ...
  // Show fallback if not on client or if Three.js has errors
  if (!isClient || hasThreeJSError) {
    return <FallbackGame walletConnected={walletConnected} />
  }
    ...
    <>
      ...
      {showUpgradeModal && (
        <UpgradeModal
          ...
          gameState={gameState}
          purchasedSubmarines={[playerTier]}
          onSelectSubmarine={() => {}}
          selectedSubmarine={playerTier}
        />
      )}
    </>
  )
