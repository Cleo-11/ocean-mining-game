...
function OtherPlayerSubmarine({ player }: OtherPlayerSubmarineProps) {
  ...
  // Render simplified version for other players (performance optimization)
  const renderSimplifiedSubmarine = () => {
    const tier = player.submarineType
      ...
      return (
        <>
          <mesh castShadow>
            ...
            <meshStandardMaterial color={submarineColor} roughness={0.2} metalness={0.8} />
          </mesh>
          <mesh position={[0, 0.6, 0]} castShadow>
            <boxGeometry args={[0.15, 0.5, 0.8]} />
            <meshStandardMaterial color={submarineColor} roughness={0.2} metalness={0.8} />
          </mesh>
          {tier >= 5 && <pointLight position={[0, 0, 0]} intensity={0.3} color={submarineColor} distance={2} />}
        </>
      )
    } else if (tier <= 10) {
      return (
        <>
          <mesh castShadow>
            ...
            <meshStandardMaterial color={submarineColor} roughness={0.1} metalness={0.9} />
          </mesh>
          <mesh position={[0, 0.8, 0]} castShadow>
            <cylinderGeometry args={[0.4, 0.6, 0.8, 16]} />
            <meshStandardMaterial color={submarineColor} roughness={0.1} metalness={0.9} />
          </mesh>
          <pointLight position={[0, 0, 0]} intensity={0.5} color={submarineColor} distance={3} />
        </>
      )
    } else {
      return (
        <>
          <mesh castShadow>
            ...
            <meshStandardMaterial color={submarineColor} roughness={0.05} metalness={0.95} />
          </mesh>
          <mesh position={[0, 0, 0]} castShadow>
            <sphereGeometry args={[0.4, 16, 16]} />
            <meshStandardMaterial
              color={submarineColor}
              emissive={submarineColor}
              emissiveIntensity={0.3}
              transparent
              opacity={0.8}
            />
          </mesh>
          <pointLight position={[0, 0, 0]} intensity={0.8} color={submarineColor} distance={4} />
        </>
      )
    }
  }
