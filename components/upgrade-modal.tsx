    ...
    <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-900/80 backdrop-blur-sm">
      <div className="w-full max-w-2xl rounded-xl bg-slate-800 p-6 shadow-2xl">
        ...
        <div className="mb-6 rounded-lg bg-slate-700 p-4">
          <h3 className="mb-3 text-lg font-semibold text-slate-200">Your Submarines</h3>
          <div className="grid grid-cols-3 gap-3">
            {purchasedSubmarines.map((tier) => {
              const sub = getSubmarineByTier(tier)
              return (
                <button
                  key={tier}
                  onClick={() => onSelectSubmarine(tier)}
                ...
                >
                  <div className="text-center">
                    <div className="mb-1 text-xl" style={{ filter: `drop-shadow(0 0 8px ${sub.color}60)` }}>
                      🚢
                    </div>
                    <div className="font-bold text-cyan-400">{sub.name}</div>
                    <div className="text-xs text-slate-300">Tier {tier}</div>
                  </div>
                </button>
              )
            })}
